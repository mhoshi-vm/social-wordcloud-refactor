...
class AnalyticsGrpcServiceTest {
	...
	void deleteMessages_ShouldCallComponentAndReturnSuccess() {
		...
		DeleteRequest request = DeleteRequest.newBuilder().addIds(testId).build();
	...
	void deleteMessages_ShouldReturnErrorOnException() {
		...
		doThrow(new RuntimeException("Database error")).when(analyticsComponent).deleteSocialMessages(anyList());
