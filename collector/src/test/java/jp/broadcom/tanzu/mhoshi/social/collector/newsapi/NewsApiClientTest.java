...
class NewsApiClientTest {
	...
	void newsApiSupplier_ShouldFetchAndSaveNextSecondAsOffset() {
		...
		NewsApiResponse.NewsApiResponseArticles article = new NewsApiResponse.NewsApiResponseArticles(source, "Author",
				"Desc", "http://news.com", publishedAt, "Content");
		...
		// 2023-10-01T10:00:00 + 1 second = 2023-10-01T10:00:01Z (Instant.toString
		// representation)
	...
	void convertNewsApiResponse_ShouldFormatContentCorrectly() {
		...
		NewsApiResponse.NewsApiResponseArticles article = new NewsApiResponse.NewsApiResponseArticles(source,
				"Wolf Blitzer", "Breaking News", url, "2023-10-01T12:00:00", "Full story content");
		...
		assertThat(msg.createDateTime())
			.isEqualTo(LocalDateTime.parse("2023-10-01T12:00:00", DateTimeFormatter.ISO_DATE_TIME));
	}
