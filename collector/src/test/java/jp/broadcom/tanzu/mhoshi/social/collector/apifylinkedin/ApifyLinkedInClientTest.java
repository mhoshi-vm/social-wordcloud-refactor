...
class ApifyLinkedInClientTest {
    ...
    void pollLinkedIn_ShouldFetchResponses() {
        ...
        ApifyLinkedInResponse.ApifyLinkedInResponseAuthor author =
                new ApifyLinkedInResponse.ApifyLinkedInResponseAuthor("Broadcom");

        ApifyLinkedInResponse.ApifyLinkedInResponsePostedAt postedAt =
                new ApifyLinkedInResponse.ApifyLinkedInResponsePostedAt(System.currentTimeMillis());

        ApifyLinkedInResponse response = new ApifyLinkedInResponse(
                "activity-1", "http://linkedin.com/post/1", "Hello World", author, postedAt);
        ...
        // Verify the client was called with the correct request parameters based on properties
        Mockito.verify(apifyLinkedInClient).apifyLinkedInResponses(
                eq(appId),
                eq(token),
                eq(new ApifyLinkedInRequest(apifyLinkedInProperties.keyword(), apifyLinkedInProperties.pollingLimit(), apifyLinkedInProperties.sortType()))
        );
    }
    ...
    void convertLinkedInResponse_ShouldConvertToSocialMessage() {
        ...
        ApifyLinkedInResponse.ApifyLinkedInResponseAuthor author =
                new ApifyLinkedInResponse.ApifyLinkedInResponseAuthor("Jane Doe");

        ApifyLinkedInResponse.ApifyLinkedInResponsePostedAt postedAt =
                new ApifyLinkedInResponse.ApifyLinkedInResponsePostedAt(timestamp);
