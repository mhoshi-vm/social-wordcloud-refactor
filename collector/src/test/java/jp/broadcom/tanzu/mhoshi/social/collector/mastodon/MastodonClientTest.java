...
class MastodonClientTest {
	...
	void pollMastodon_ShouldFetchAndSaveOffset() {
		...
		MastodonTimelinesResponse response1 = new MastodonTimelinesResponse("100", "Content 1", "en", "http://url1",
	...
	void convertMastodonTimelinesResponse_ShouldConvertToSocialMessage() {
		...
		MastodonTimelinesResponse.MastodonTimelinesResponseAccount account = new MastodonTimelinesResponse.MastodonTimelinesResponseAccount(
				"user", "User Name");
		MastodonTimelinesResponse response = new MastodonTimelinesResponse("123", "Some content", "en",
				"http://example.com/123", now, account);
	...
	void mastodonTimelinesResponses_ShouldRecursivelyFetch_WhenLimitIsReached() {
		...
		// Mock the recursive call: maxId should be the ID of the last item from page 1
		// ("99")
		Mockito.when(mastodonClient.getMastodonTimeLineResponses(eq(hashtag), eq(limit), eq(sinceId), eq("99")))
			.thenReturn(page2);

		// Act
		// We call the package-private method directly since we are in the same package in
		// src/test
		List<MastodonTimelinesResponse> result = mastodonClientConfig.mastodonTimelinesResponses(mastodonClient,
				hashtag, limit, sinceId, null);
