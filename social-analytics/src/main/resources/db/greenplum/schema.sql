...
-- Note: Removed PK and FK. In Greenplum, enforce uniqueness via index if needed,
-- but PKs on AO tables have strict requirements.
...
CREATE MATERIALIZED VIEW IF NOT EXISTS term_frequency_entity with(mv_maintain_mode=full) AS
...
--- 7. Aggregation Layer
CREATE MATERIALIZED VIEW hourly_message_stats with(mv_maintain_mode=full) AS
SELECT
...
CREATE MATERIALIZED VIEW daily_stock_metrics with(mv_maintain_mode=full) AS
SELECT
