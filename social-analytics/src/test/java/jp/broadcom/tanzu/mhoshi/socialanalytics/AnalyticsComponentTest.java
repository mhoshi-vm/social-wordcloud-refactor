...
class AnalyticsComponentTest {
    ...
    void testFullMessageLifecycle() {
        ...
        SocialMessage message = new SocialMessage(
                msgId,
                "TestNews",
                "This is a positive test message about AI technology.",
                "en",
                "Test User",
                "http://example.com",
                LocalDateTime.now()
        ...
        // 4. Update Sentiment
        // Note: The postgres version of vaderSentimentFunction.py provided returns 0.0 dummy value
    ...
    void testGisInfoUpdate() {
        // 1. Insert Message
        String msgId = UUID.randomUUID().toString();
        SocialMessage message = new SocialMessage(msgId, "GeoSource", "Event at Tokyo Tower", "en", "User", "url", LocalDateTime.now());
        analyticsComponent.insertSocialMessages(List.of(message));

        // 2. Mock AI GIS Response
        GisInfo mockGis = new GisInfo(msgId, message.createDateTime(), 4326, "POINT(139.7454 35.6586)", "Tokyo Tower location");
